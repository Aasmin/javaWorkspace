...
public class l001{
   ...
   public static void postOrder(Node node){
    ...
    public static boolean lowestCommonAncestor_(Node node, int p, int q) {
        if(node == null) return false;

        boolean selfdone = false;
        if(node.data == p || node.data == q)        selfdone = true;

        boolean lnode = lowestCommonAncestor_(node.left, p, q);
        if(LCANode != null) return true;

        boolean rnode = lowestCommonAncestor_(node.right, p, q);
        if(LCANode != null) return true;

        if((lnode && rnode) || (lnode && selfdone) || (rnode && selfdone))
            LCANode = node;

        return selfdone || lnode || rnode;
    }
    ...
    public void kDown(Node node, int K, Node block, List<Integer> ans) {
    ...
    public boolean nodeToRootPath(Node node, int target, ArrayList<Node> path) {
        if(node == null)    return false;
    ...
    public static int nodeToRootPath_03(Node node, int target, int k) {
        if(node.data == target) {
            kDown(node, k);
            return 1;
        }
        ...
        if(leftDistance != -1) {
            if(k - leftDistance == 0) System.out.println(node.data + " ");
        ...
        if(rightDistance != -1) {
            if(k - rightDistance == 0) System.out.println(node.data + " ");
